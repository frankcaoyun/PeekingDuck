...
def init_inferer(weights=osp.join(ROOT, 'yolov6s.pt'),
    ...
    # Inference
    # the source is now an image
    
    # inferer = Inferer(source, weights, device, yaml, img_size, half)
    inferer = Inferer(weights, device, yaml, img_size, half)
    # bboxes, classes, scores = inferer.infer(conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf)
    
    # if save_txt or save_img:
    #     LOGGER.info(f"Results saved to {save_dir}")
    
    # return the values from interer, pass to model
...
def run(
    ...
    # inferer = Inferer(source, weights, device, yaml, img_size, half)
    # bboxes, classes, scores = inferer.infer(conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf)
    bboxes, classes, scores = inferer.infer(source, conf_thres, iou_thres, classes, agnostic_nms, max_det
